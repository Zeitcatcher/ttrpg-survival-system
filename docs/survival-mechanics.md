...
| 5 | **Climate model** | `Off` / `Manual band` / `Read weather module` | **Manual band** | `Off` disables water-scaling and the cold track (temperate campaigns). Manual = GM picks a band per region/group. Auto = read a weather/calendar module (e.g. Seasons & Stars, v13/v14) if installed. |
