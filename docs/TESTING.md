...
- **Native pf2e Rations = FOOD, counted by their real charges** (v0.6.1): the module reads `system.uses` — a Rations item at **1 of 7 charges counts as 1 food-day, not 7**. Eat charges on the sheet and the pool drops to match; the module's own consume/grant write the charge counter back too. A stack with no charge counter falls back to 7/week.
- **Live updates (v0.6.1):** with the Survival window open, changing rations/water on a character sheet updates the Pools list immediately — no reopen needed.
...
- The chat line reports how much water was made and from how many castings. Settings: *Water spells* (on/off), *Water per cast*, *Water spell slugs*. Prompts fire from the panel's **Advance** buttons (world-clock ticks don't prompt).
